...
from miaonlp.tf.utils import gelu
from miaonlp.tf.layers import PositionEmbedding, SelfAttentionMask, TransformerEncoderBlock, OnDeviceEmbedding
...
class AlbertEncoder(tf.keras.Model):
    ...
    *Note* that the network is constructed by Keras Functional API.
    ...
    def __init__(self,
        ...
        type_embeddings = (
            OnDeviceEmbedding(vocab_size=type_vocab_size,
                              embedding_width=embedding_width,
                              initializer=initializer,
                              use_one_hot=True,
        ...
        shared_layer = TransformerEncoderBlock(
        ...
        super(AlbertEncoder, self).__init__(
            inputs={'input_word_ids': word_ids, 'input_mask': mask, 'input_type_ids': type_ids}, outputs=outputs,
            **kwargs)
        config_dict = {
            'vocab_size': vocab_size,
            'embedding_width': embedding_width,
    ...
    def load_encoder_weights_from_google_ckpt(self, ckpt_path):
        variables = tf.train.load_checkpoint(ckpt_path)

        self.get_layer("word_embeddings").set_weights([
            variables.get_tensor("bert/embeddings/word_embeddings")])
        self.get_layer("position_embedding").set_weights([
        ...
        self.get_layer("transformer").set_weights([
            tf.reshape(
                variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/attention_1/self/query/kernel"),
                [self.hidden_size, self.num_attention_heads, -1]),
            tf.reshape(
                variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/attention_1/self/query/bias"),
                [self.num_attention_heads, -1]),
            tf.reshape(
                variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/attention_1/self/key/kernel"),
                [self.hidden_size, self.num_attention_heads, -1]),
            tf.reshape(
                variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/attention_1/self/key/bias"),
                [self.num_attention_heads, -1]),
            tf.reshape(
                variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/attention_1/self/value/kernel"),
                [self.hidden_size, self.num_attention_heads, -1]),
            tf.reshape(
                variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/attention_1/self/value/bias"),
                [self.num_attention_heads, -1]),
            tf.reshape(
                variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/attention_1/output/dense/kernel"),
                [self.num_attention_heads, -1, self.hidden_size]),
            ...
            variables.get_tensor("bert/encoder/transformer/group_0/inner_group_0/ffn_1/intermediate/output/dense/bias"),
...
if __name__ == '__main__':
    model = AlbertEncoder(vocab_size=21128)

    model.load_encoder_weights_from_google_ckpt("../data/albert_base/model.ckpt-best")
    model.vocab_file = tf.saved_model.Asset("../data/albert_base/vocab_chinese.txt")
    model.save("../data/hub_models/albert_base_zh", include_optimizer=False, save_format="tf")
